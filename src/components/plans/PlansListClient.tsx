...
export default function PlansListClient({ plans }: { plans: Plan[] }) {
                ...
                <Link href={`/plans/${plan.id}`} className="block">
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-label-sm text-on-surface font-medium ml-2">{formatCurrency(plan.contribution_amount)}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-label-sm text-secondary">Cycle</span>
                      <span className="text-label-sm text-on-surface font-medium ml-2">{formatCycleDuration(plan)}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-label-sm text-secondary">Members</span>
                      <span className="text-label-sm text-on-surface font-medium ml-2">{memberCount}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-label-sm text-secondary">Expected</span>
                      <span className="text-label-sm text-on-surface font-medium ml-2">{formatCurrency(totalExpected)}</span>
                    </div>
                    <div className="flex items-center justify-between pt-1.5 border-t border-surface-variant mt-1">
                      <span className="text-label-sm text-secondary">Payout/Cycle</span>
                      <span className="text-label-sm text-[#059669] font-medium ml-2">{formatCurrency(plan.payout_amount)}</span>
                    </div>
                  </div>
                </Link>
              </div>
